...
exports.criar = async (req, res) => {
    try {
        const { nome, email, senha, cpf, data_nascimento, telefone, url_foto, is_admin } = req.body;
        ...
        if(erro.code === '23505') {
            return res.status(400).json({ mensagem: 'E-mail ou CPF já estão cadastrados.' });
        }
...
exports.atualizarUsuario = async (req, res) => {
    try {
        ...
        if (senha && senha.trim() !== '') {
            ...
            const usuarioCompleto = await UsuarioRepository.buscarPorEmail(usuarioExistente.email);
        ...
        const usuarioAtualizado = await UsuarioRepository.atualizarUsuario(dadosAtualizados);
